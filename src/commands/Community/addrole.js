...
module.exports = {
    name: 'role-add',
    description: 'This command adds a specific role to a user.',
    ...
    async execute(message, args, client) {
        ...
        if (!user) return message.channel.send('You must mention a user to add a role to.');

        const roleMention = message.mentions.roles.first();
        if (!roleMention) return message.channel.send('You must specify a valid role to add.');

        const role = message.guild.roles.cache.get(roleMention.id);
        if (!role) return message.channel.send('You must specify a valid role to add.');
        ...
        if (role.position >= requestRolePosition) {
            const rankErrorEmbed = new EmbedBuilder()
                .setColor('#f04a47')
                .setDescription('<:dynoError:1326926743650435166> You cannot give a role equal to or higher than yours.');
        ...
        try {
            ...
            const successEmbed = new EmbedBuilder()
                .setColor('#43b582')
                .setDescription(`***<:dynoSuccess:1326926745584013362> The ${role.name} role has been added to ${targetUser.user.tag}.***`);
